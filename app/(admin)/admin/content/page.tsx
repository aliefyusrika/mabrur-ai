...
export default function AdminContent() {
  ...
  return (
    <AdminLayout 
      title="Konten Chatbot"
      breadcrumb={[{ label: 'Dashboard', href: '/admin/dashboard' }, { label: 'Konten Chatbot' }]}
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0">
          {['ALL', 'FAQ', 'DOA', 'LOCATION', 'INFO'].map((type) => (
            <button
              key={type}
          ...
          <AdminCard key={content.id}>
            <div className="flex items-start justify-between gap-3 mb-3">
              ...
              <div className="flex gap-3">
                <Link href={`/admin/content/${content.id}`} className="text-sm text-navy-600 hover:text-navy-800">
                  Edit
                </Link>
            ...
            <h3 className="font-medium text-navy-900 mb-2">{content.question}</h3>
            <p className="text-gray-500 text-sm line-clamp-2">{content.answer}</p>
          </AdminCard>
        ))}
      </div>

      {filteredContents.length === 0 && (
        <div className="text-center py-12 text-gray-400">Belum ada konten</div>
      )}
    </AdminLayout>
  )
}
